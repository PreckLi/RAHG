...
def get_edge_index(file):
    ...
    edge_list = list(map(list, zip(*edge_list)))  # 转置
...
def set_mask(data: Data, args):
    ...
    if args.dataset == '295' or args.dataset == '296' or args.dataset == '297':
        d = 5
    if args.dataset == '123':
        d = 3
    if args.dataset != '118':
        sample_patience = 0
        train_sample_patience = 0
        for idx, i in enumerate(data.y):
            if i == 0:
                if train_sample_patience % 4 != 0:
                    train_mask[idx] = True
                if train_sample_patience % 4 == 0:
                    test_mask[idx] = True
            if i == 1:
                if train_sample_patience % 4 != 0 and sample_patience % d == 0:
                    train_mask[idx] = True
                if train_sample_patience % 4 == 0 and sample_patience % d == 0:
                    test_mask[idx] = True
                sample_patience += 1
            train_sample_patience += 1
    if args.dataset == '118':
