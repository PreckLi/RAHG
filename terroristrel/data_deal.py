...
def load_data(args):
    edge_file = f'datasets/terroristrel/terroristrel.edgelist'
    label_file = f'datasets/terroristrel/label.txt'
    edge_index = get_edge_index(edge_file)
    label = get_label(label_file)
    if args.emb_type != 'attention':
        if args.emb_type == 'graphwave':
            x_file = f'datasets/graphwave_emb/terroristrel.csv'
        if args.emb_type == 'node2vec':
            x_file = f'datasets/node2vec_emb/terroristrel.emb'
        ...
        data = Data(x=x, edge_index=edge_index, num_nodes=num_nodes, num_node_features=num_node_features, y=label,
                    num_classes=2)
    if args.emb_type == 'attention':
        emb_role_file = f'datasets/graphwave_emb/terroristrel.csv'
        emb_stru_file = f'datasets/node2vec_emb/terroristrel.emb'
